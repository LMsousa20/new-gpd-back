...
router.get("/", async (req, res) => {
  try {
    const retorno = await dbOne.sequelize.query("Select * from tickets");
...
router.post("/", async function (req, res) {
  ...
  const { fantasia, cnpj, motivo, tecnico, previsão } = req.body
  console.log(fantasia, cnpj, motivo, tecnico, previsão)
  
  const retorno = await dbOne.Ticket.create({
    fantasia: fantasia,
    cnpj: cnpj,
    motivo: motivo,
    tecnico: tecnico,
